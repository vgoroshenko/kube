...
curl -s -X PATCH --data '{"spec":{"template":{"metadata":{"annotations":{"kubectl.kubrnetes.io/restartedAt":"'"$(date +%Y-%m-%dT%T%z)"'"}}}}}' -H "Accept: application/json, */*" -H "Content-Type: application/strategic-merge-patch+json" $APISERVER/apis/apps/v1/namespaces/default/deployments/mlnui  --header "Authorization: Bearer $TOKEN" --cacert ./ca.crt

...
echo "$(echo "$SECRET_OUTPUT" | jq .data.data.value | sed 's/"//g' | base64 -d)" >  ./ca.crt
#echo "$SECRET_OUTPUT" | jq .data.data.urlmlncs | sed 's/"//g' | base64 -d | sh
APISERVER=$(echo "$SECRET_OUTPUT" | jq .data.data.apiserver | sed 's/"//g')
TOKEN=$(echo "$SECRET_OUTPUT" | jq .data.data.token | sed 's/"//g')
curl -s -X PATCH --data '{"spec":{"template":{"metadata":{"annotations":{"kubectl.kubrnetes.io/restartedAt":"'"$(date +%Y-%m-%dT%T%z)"'"}}}}}' -H "Accept: application/json, */*" -H "Content-Type: application/strategic-merge-patch+json" $APISERVER/apis/apps/v1/namespaces/default/deployments/mlncs-dev  --header "Authorization: Bearer $TOKEN" --cacert ./ca.crt
